...
async function main() {
  ...
  // Build name → existing-target-id map so we can detect conflicts AND reuse
  // existing parents that may already be on the target.
  console.log('scanning target for existing tags...');
  const targetByName = new Map<string, number>();
  for await (const tag of client.iterateAllTags()) {
    if (tag.id != null) targetByName.set(tag.name, tag.id);
  }
  console.log(`target has ${targetByName.size} existing tags\n`);

  const ordered = topologicalOrder(exportFile.tags);

  // Maps source tag id → id on the target (either newly created or pre-existing).
